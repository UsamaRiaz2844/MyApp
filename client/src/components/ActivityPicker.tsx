import { ACTIVITIES, activityString } from '../lib/activity';

interface Props {
  current: string | null;
  onPick: (activity: string | null) => void;
  onClose: () => void;
}

// Bottom sheet to set your "what are you up to?" status (or clear it).
export default function ActivityPicker({ current, onPick, onClose }: Props) {
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <h2 className="mb-3 text-center text-base font-bold text-slate-900 dark:text-white">What are you up to?</h2>
        <div className="grid grid-cols-2 gap-2">
          {ACTIVITIES.map((a) => {
            const s = activityString(a);
